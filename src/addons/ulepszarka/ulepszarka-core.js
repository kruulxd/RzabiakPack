...
(function () {
  ...
  const Ui = {
    setupCss() {
      ...
      .menu-item--yellow {
        background: linear-gradient(90deg, #1f7a3d, #22c55e) !important;
      ...
      .upgrader-label {
          ...
          .upgrader-launcher {
            ...
            width: 146px;
            z-index: 12;
            border: 1px solid rgba(52, 211, 100, 0.34);
            border-radius: 10px;
            background: linear-gradient(180deg, rgba(5, 13, 7, 0.96), rgba(8, 23, 12, 0.95));
            color: #d4f0dc;
          ...
          .upgrader-gui-panel {
            ...
            border-radius: 12px;
            background: linear-gradient(180deg, rgba(6, 16, 10, 0.97), rgba(11, 28, 16, 0.95));
          ...
          .upgrader-gui-title {
            ...
            border-bottom: 1px solid rgba(52, 211, 100, 0.24);
            color: #86efac;
          }
          ...
          .upgrader-gui-btn:hover {
            ...
            .upgrader-mode-wrap {
              ...
              border-radius: 8px;
              padding: 8px;
              background: linear-gradient(180deg, rgba(34, 197, 94, 0.08), rgba(13, 35, 19, 0.15));
            }
            .upgrader-mode-row {
              display: flex;
              align-items: center;
            ...
            .upgrader-mode-slider {
              ...
              inset: 0;
              border-radius: 24px;
              border: 1px solid rgba(52, 211, 100, 0.35);
            ...
            .upgrader-tooltip-trigger {
              ...
              height: 14px;
              border-radius: 50%;
              border: 1px solid rgba(52, 211, 100, 0.42);
              color: #d4f0dc;
              font-size: 10px;
              line-height: 1;
              background: rgba(34, 197, 94, 0.1);
            ...
            .upgrader-tooltip-trigger[data-tooltip]:hover::after {
              ...
              border-radius: 6px;
              border: 1px solid rgba(52, 211, 100, 0.24);
              background: linear-gradient(180deg, rgba(6, 16, 10, 0.96), rgba(8, 19, 12, 0.96));
              color: #d4f0dc;
              font-size: 10px;
              line-height: 1.35;
              white-space: normal;
  ...
  span.className = rarityMeta[rarity]?.className || "";
    ...
    removeInterfaceWidget() {
      ...
      if (state.interfaceWidgetDragObserver) {
        state.interfaceWidgetDragObserver.disconnect();
        state.interfaceWidgetDragObserver = null;
      }
  ...
  const ExternalApi = {
    ensureUiCreated() {
      if (!document.getElementById("upgrader-launcher")) {
        Ui.createGui();
      }
    },

    openSettingsPanel() {
      ExternalApi.ensureUiCreated();
    ...
    toggleSettingsPanel(forceVisible = null) {
      ...
      if (typeof forceVisible === "boolean") {
        if (forceVisible && !state.guiVisible) {
          Ui.toggleGui();
          return true;
        }
    ...
    toggleWidgetLauncher(forceVisible = null) {
      ExternalApi.ensureUiCreated();

      Ui.toggleLauncherVisibility(forceVisible);

      const panel = document.getElementById("upgrader-gui-panel");
      if (panel && forceVisible === true) {
    ...
    showWidgetLauncher() {
      return ExternalApi.toggleWidgetLauncher(true);
    },

    hideWidgetLauncher() {
      return ExternalApi.toggleWidgetLauncher(false);
    },
  };

  window.__RZP_ULEPSZARKA_NI_API = ExternalApi;
